...
namespace CarPartsPatron.Auth
{
    public class FirebaseAuthService : IFirebaseAuthService
    {
        ...
        {
            //library creates http clients to use http requests
        ...
        private async Task<FirebaseUser> SignUpOrSignIn(string email, string password, string url)
        {
            ...
            var firebaseResponse =
                JsonSerializer.Deserialize<FirebaseResponse>(content, _jsonSerializerOptions); // converts to dotnet object

            return firebaseResponse.LocalId != null
                ? new FirebaseUser(firebaseResponse.Email, firebaseResponse.LocalId)
                : null; //checks if id is found otherwise null to pass to null handle function
        }
    }
}
